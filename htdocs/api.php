<?php
    ...
    require_once 'COS216/PA4/backend/config.php';
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $reqTypes = ["info","update","login","rate","chat"];
    ...
    class API {
        ...
        function getArticles($instance, $title, $author, $date, $rating, $types) {
            $instance->populateDB();
        ...
        function setPreferences($instance, $key, $category, $tags) {
            $catUpdate = false;
            $tagsUpdate = false;

            if ($category != "") {
                $query = "UPDATE `users` SET `category` = '$category' WHERE `api` = '$key'";
                $instance->getConn()->query($query);
                $catUpdate = true;
            }

            if ($tags != "") {
                $query = "UPDATE `users` SET `tag` = '$tags' WHERE `api` = '$key'";
                $instance->getConn()->query($query);
                $tagsUpdate = true;
            }

            $resp =  ["status"=>"success", "timestamp"=>time(), "data"=>["category"=>$category, "tag"=>$tags], "updated"=>["category"=>$catUpdate, "tag"=>$tagsUpdate]];
            header("HTTP/1.1 200 OK");
		    header("Content-Type: application/json");
            echo json_encode($resp);
        }
